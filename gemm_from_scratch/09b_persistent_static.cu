...
constexpr int NUM_WARPS = 10;  // 1 tma, 1 mma, 8 epilogue
...
void kernel(bf16 *A, bf16 *B, bf16 *C, int M, int N, int K,
    ...
    else if (warp_id >= 2) { // 8 epilogue warps (2 warpgroups of 4)
        int epi_warp_id = warp_id % 4; 
        int epi_idx = (warp_id - 2) / 4;  // warpgroups 0 uses epi_idx=0, warpgroups 1 uses epi_idx=1
        int epi_phase = 0;

        for (int cur_bid = bid + epi_idx * total_bids; cur_bid < num_tiles; cur_bid += 2 * total_bids) {
            int2 tile = compute_position(cur_bid, cta_rank, grid_m, grid_n);
            int tile_m = tile.y;
            int tile_n = tile.x;
            ...
            for (int n = 0; n < BLOCK_N / 8; n++) {
                ...
                const int addr = taddr + (((cta_rank * 128) + epi_warp_id * 32) << 16) + (epi_idx * BLOCK_N + n * 8);
                ...
                bf16 *out_ptr = C + (off_m + epi_warp_id * 32 + lane_id) * N + (off_n + n * 8);
            ...
            epi_phase ^= 1;  // toggle phase for next tile
        }
    }
