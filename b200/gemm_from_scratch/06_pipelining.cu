/* 
added pipelining (without warp specialization)
522 tflops
...
OUTLINE:
num_stages = 2
A_smem_addrs[num_stages]
B_smem_addrs[num_stages]
mbar_addrs[num_stages]
phase = 0

issue tma for tile 0

for iterk = 0; k < K / block_k 
    issue tma for (iterk + 1) % 2 (if in bounds of total tiles)

    wait on mbar[iterk] for iterk for that tile to be filled, then phase^=1
    compute 
    wait on compute to finish on mbar[iterk], then phase^=1
...
// really only works for 2 stages since i wanted to move onto warp specialization
// should be easy to extend, but we're near the smem limit anyway
...
void kernel(bf16 *A, bf16 *B, bf16 *C, int M, int N, int K,
    ...
    __shared__ alignas(1024) bf16 A_tile[BLOCK_M * BLOCK_K * STAGES];
    __shared__ alignas(1024) bf16 B_tile[BLOCK_N * BLOCK_K * STAGES];
    int A_smem[STAGES];
    int B_smem[STAGES];
    A_smem[0] = static_cast<int>(__cvta_generic_to_shared(A_tile));
    A_smem[1] = static_cast<int>(__cvta_generic_to_shared(A_tile + (BLOCK_M * BLOCK_K)));
    B_smem[0] = static_cast<int>(__cvta_generic_to_shared(B_tile));
    B_smem[1] = static_cast<int>(__cvta_generic_to_shared(B_tile + (BLOCK_N * BLOCK_K)));

    __shared__ uint64_t mbar[2]; // mbarrier
    int mbar_addrs[STAGES];
    mbar_addrs[0] = static_cast<int>(__cvta_generic_to_shared(mbar));
    mbar_addrs[1] = static_cast<int>(__cvta_generic_to_shared(mbar + 1));
    ...
    if (warp_id == 0 && is_elect_sync()) {
        ...
        asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;" :: "r"(mbar_addrs[0]), "r"(1));
        asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;" :: "r"(mbar_addrs[1]), "r"(1));
    ...
    // Issue TMA for tile 0 before the loop
    if (warp_id == 0 && is_elect_sync()) {
        int off_m = tile_m * BLOCK_M;
        int off_n = tile_n * BLOCK_N;
        for (int k = 0; k < BLOCK_K / CORE_MATRIX_WIDTH; k++) {
            int off_k = k * CORE_MATRIX_WIDTH;  // tile 0, so t=0
            tma_2d_gmem2smem(A_smem[0] + k * BLOCK_M * CORE_MATRIX_WIDTH_BYTES, &A_tmap, off_k, off_m, mbar_addrs[0]);
            tma_2d_gmem2smem(B_smem[0] + k * BLOCK_N * CORE_MATRIX_WIDTH_BYTES, &B_tmap, off_k, off_n, mbar_addrs[0]);
        }
        constexpr int copy_size = (BLOCK_M + BLOCK_N) * BLOCK_K * sizeof(bf16);
        asm volatile("mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;"
            :: "r"(mbar_addrs[0]), "r"(copy_size) : "memory");
    }

    for (int t = 0; t < num_k_tiles; t++) {
        
        if (warp_id == 0 && is_elect_sync() && t < num_k_tiles - STAGES + 1) {
            int tma_idx = (t + 1) % 2;
            int off_m = tile_m * BLOCK_M;
            int off_n = tile_n * BLOCK_N;

            for (int k = 0; k < BLOCK_K / CORE_MATRIX_WIDTH; k++) {
                int off_k = (t + 1) * BLOCK_K + k * CORE_MATRIX_WIDTH;
                tma_2d_gmem2smem(A_smem[tma_idx] + k * BLOCK_M * CORE_MATRIX_WIDTH_BYTES, &A_tmap, off_k, off_m, mbar_addrs[tma_idx]);
                tma_2d_gmem2smem(B_smem[tma_idx] + k * BLOCK_N * CORE_MATRIX_WIDTH_BYTES, &B_tmap, off_k, off_n, mbar_addrs[tma_idx]);
            }
            
            constexpr int copy_size = (BLOCK_M + BLOCK_N) * BLOCK_K * sizeof(bf16);
            asm volatile("mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;"
                :: "r"(mbar_addrs[tma_idx]), "r"(copy_size) : "memory");
        }

        int cur_idx = t % 2;
        mbarrier_wait(mbar_addrs[cur_idx], phase);
        phase ^= 1;

        // don't let my later tcgen05 mma get reordered before the wait we just did
        asm volatile("tcgen05.fence::after_thread_sync;");

        // Issue MMA operations (only 1 thread)
        if (warp_id == 0 && is_elect_sync()) {
            // Manually unroll 1st iteration to disable accumulation
            uint64_t a_desc = make_smem_desc_swizzled(A_smem[cur_idx]);
            uint64_t b_desc = make_smem_desc_swizzled(B_smem[cur_idx]);
            tcgen05_mma_f16(taddr, a_desc, b_desc, IDESC, t);
            for (int k2 = 1; k2 < CORE_MATRIX_WIDTH / MMA_K; k2++) {
                int a_addr = A_smem[cur_idx] + k2 * MMA_K * 2;
                int b_addr = B_smem[cur_idx] + k2 * MMA_K * 2;
            ...
            for (int k1 = 1; k1 < BLOCK_K / CORE_MATRIX_WIDTH; k1++) {
                for (int k2 = 0; k2 < CORE_MATRIX_WIDTH / MMA_K; k2++) {
                    int a_addr = A_smem[cur_idx] + k1 * CORE_MATRIX_WIDTH_BYTES * BLOCK_M + k2 * MMA_K * 2;
                    int b_addr = B_smem[cur_idx] + k1 * CORE_MATRIX_WIDTH_BYTES * BLOCK_N + k2 * MMA_K * 2;
                    uint64_t a_desc = make_smem_desc_swizzled(a_addr);
                    uint64_t b_desc = make_smem_desc_swizzled(b_addr);
                    tcgen05_mma_f16(taddr, a_desc, b_desc, IDESC, 1);
                }
