...
constexpr uint32_t CORE_MATRIX_WIDTH_BYTES = 16;
constexpr uint32_t CORE_MATRIX_WIDTH = CORE_MATRIX_WIDTH_BYTES / sizeof(bf16); // 8 bf16 elements
...
void kernel(bf16 *A, bf16 *B, bf16 *C, int M, int N, int K,
    ...
    __shared__ alignas(128) bf16 A_tile[BLOCK_M * BLOCK_K];
    __shared__ alignas(128) bf16 B_tile[BLOCK_N * BLOCK_K];
    ...
    for (int t = 0; t < num_k_tiles; t++) {
        ...
        if (warp_id == 0 && is_elect_sync()) {
            ...
            // TMA load: issue BLOCK_K/8 loads, each loading 8 bf16 columns (one core matrix width)
            // This arranges data in shared memory in core matrix layout for tcgen05.mma
        ...
        if (warp_id == 0 && is_elect_sync()) {
            // Inner loop: BLOCK_K / MMA_K = 256 / 16 = 16 MMA operations
            for (int k = 0; k < BLOCK_K / MMA_K; k++) {
                // Each MMA_K = 16 bf16 = 2 core matrices (each 8 wide)
                // Stride by k * 2 core matrices worth of bytes
                int a_addr = A_smem + k * 2 * BLOCK_M * CORE_MATRIX_WIDTH_BYTES;
                int b_addr = B_smem + k * 2 * BLOCK_N * CORE_MATRIX_WIDTH_BYTES;
                
                uint64_t a_desc = make_smem_desc(a_addr, BLOCK_M);
                uint64_t b_desc = make_smem_desc(b_addr, BLOCK_N);
                
                // accumulate = 1 except for first MMA of first K-tile
                int accumulate = (t > 0 || k > 0) ? 1 : 0;
                tcgen05_mma_f16(taddr, a_desc, b_desc, IDESC, accumulate);
            }
...
int main() {
    ...
    CUtensorMap A_tmap{};
    CUtensorMap B_tmap{};
    CUresult res1 = init_tmap_2d(&A_tmap, A_d, K, M, CORE_MATRIX_WIDTH, BLOCK_M, CU_TENSOR_MAP_SWIZZLE_NONE);
    CUresult res2 = init_tmap_2d(&B_tmap, B_d, K, N, CORE_MATRIX_WIDTH, BLOCK_N, CU_TENSOR_MAP_SWIZZLE_NONE);
